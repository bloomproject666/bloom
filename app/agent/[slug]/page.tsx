      ...
      <main className="flex-1">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
          ...
          <div className="flex items-center gap-2 text-sm text-[#6B7280] mb-5">
            ...
            <span>/</span>
            <span className="text-[#111111] truncate">{agent.name}</span>
          </div>

          {/* Title block */}
          <div className="mb-4">
            <div className="flex items-start gap-3 flex-wrap">
              <h1 className="text-xl sm:text-2xl font-bold text-[#111111]">{agent.name}</h1>
              <span className="text-xs font-mono border border-[#E5E7EB] px-2 py-1 rounded text-[#6B7280] mt-0.5">
                v{agent.version}
              </span>
            </div>
            <p className="text-[#6B7280] mt-2 leading-relaxed text-sm sm:text-base">{agent.description}</p>
          </div>
          ...
          {/* Main grid — sidebar on top in mobile, right on desktop */}
          <div className="grid lg:grid-cols-[1fr_280px] gap-8">
            ...
            <div className="order-first lg:order-last space-y-4">
              ...
              {/* Star + Stats — side by side on mobile */}
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-1 sm:gap-4">
                ...
                {/* Stats */}
                <div className="border border-[#E5E7EB] rounded-lg p-4 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-[#6B7280]">Stars</span>
                    <span className="text-xs font-semibold text-[#111111]">{agent._count.stars}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-[#6B7280]">Downloads</span>
                    <span className="text-xs font-semibold text-[#111111]">{agent.downloads}</span>
                  </div>
                  <div className="flex items-center justify-between">
                ...
                <Link href={`/u/${agent.author.username}`} className="flex items-center gap-3 group">
                  {agent.author.avatar ? (
                    <img
                      ...
                      className="w-9 h-9 rounded-full border border-[#E5E7EB] shrink-0"
                    />
                  ) : (
                    <div className="w-9 h-9 rounded-full bg-[#F3F4F6] border border-[#E5E7EB] flex items-center justify-center text-sm font-semibold text-[#6B7280] shrink-0">
            ...
            <div className="order-last lg:order-first">
              ...
              <div className="border-t border-[#E5E7EB] pt-6 mb-8">
                <h2 className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-4">README</h2>
              ...
              <div className="border-t border-[#E5E7EB] pt-6">
                <h2 className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-4">Agent Schema</h2>
                <pre className="bg-[#111111] text-[#f5f5f5] p-4 sm:p-5 rounded-lg overflow-x-auto text-xs sm:text-sm font-mono leading-relaxed">
